...
@Component({
    selector: 'orders-no-time-series',
    templateUrl: './orders-no-time-series.component.html',
...
export class OrdersNoTimeSeriesComponent implements AfterViewInit {
    ...
    private createGraph(data: DisplayData[]): void {
        this.ordersNumber.nativeElement.innerHTML = '';
        ...
        const parent = d3.select('#orders_number_time_series');
            ...
            case 'day':
                this.dataSeries.forEach(row => {
                    ...
                    if (dataMap.has(formatDate)) {
                        ...
                        dataMap.set(formatDate, val + 1);
                    } else {
                        dataMap.set(formatDate, 1);
                    }
                });
